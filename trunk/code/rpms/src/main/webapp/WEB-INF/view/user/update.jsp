...
<script type="text/javascript">
	$(function() {
		...
		$('#feeder-update-form').form({
			url : getContentPath()+'/user/update.do',
			onSubmit : function() {
        ...
        // 勾选已有的角色信息
        var roles = rowData.roleEntities;
        if (roles && roles.length > 0) {
            for (var i = 0; i < roles.length; i++) {
                var code = roles[i].code;
                $("#feeder-update-form").find("input[type=checkbox][value='" + code + "']").attr('checked', true);
            }
        }
	});
</script>
<form id="feeder-update-form" method="post" style="margin-top: 10px;">
    <table class="layout-table" >
        <input type="hidden" name="id">
        <tr>
            <td style="text-align: right"><span style="color: red;">*</span>用户名：</td>
            <td>
                <input name="username" type="text" readonly disabled>
                <span style="color: red;margin-left: 2px;">用户名不能修改</span>
            </td>
        </tr>
        <tr>
