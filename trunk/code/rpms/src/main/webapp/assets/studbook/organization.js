...
function OrgNode() {
    ...
    this.inIt = function () {
        ...
        if (this.containerId != undefined && this.containerId != null)
            getById(this.containerId).appendChild(tempDiv);
        else
...
function OrgShow(OrgNode_) {
    ...
    this.ShowType = null;
    // 自定义放树形结构的容器，默认为body
    this.showContainer = document.body;
    this.Run = function () {
        BoxInit(OrgNode_);
        GetDepth(OrgNode_);
    ...
    function BoxInit(OrgObj_) {//节点初始化
        ...
        OrgObj_.containerId = This.containerId;
        OrgObj_.inIt();
    ...
    function CreateLine(type_, top_, left_, long_, size_, color_, id_) {
        ...
        if (This.containerId != undefined && This.containerId != null)
            getById(This.containerId).appendChild(this.Line);
        else
