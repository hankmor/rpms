...
$(function () {
    ...
    var examId = $('#facade-exam-id').val();
    $('#facade-upload').click(function () {
        parent.$.modalDialog({
            title: '添加体检外观特征',
            width: 500,
            height: 600,
            href: getContentPath() + '/exam/toFacadeUploadPage.do?id=' + examId,
            buttons: [
                {
                    text: '确定',
                    iconCls: 'icon-ok',
    ...
    $('.delete-btn').click(function () {
        ...
        Dialog.confirm("该操作无法恢复，确定删除？", function () {
            ajax(getContentPath() + "/exam/deleteFacade.do", {
                examId: examId,
    ...
    $('.animal-photo').click(function () {
        parent.$.modalDialog({
            ...
            href: getContentPath() + '/exam/toShowFacadePage.do?examId=' + examId
        });
    });
});